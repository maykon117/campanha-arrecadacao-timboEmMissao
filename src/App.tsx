...
import type { AppData, Donation, Food, PackageContentUnit } from './types';
import { donatedQty, foodKg, foodMetaKg, foodPackagingLabel, foodPercent, fmt, isVolumeContentUnit, packageContentToGrams } from './utils/calculations';
import { loadData, saveData, subscribeToChanges } from './services/supabase';

const units = ['pacotes', 'unidades', 'caixas', 'sacos', 'latas', 'garrafas'];
const contentUnits: PackageContentUnit[] = ['g', 'kg', 'ml', 'L'];
...
function FoodForm({selected,onSubmit}:{selected:Food|null;onSubmit:(event:React.FormEvent<HTMLFormElement>)=>void}) {
  const [contentUnit,setContentUnit]=useState<PackageContentUnit>(selected?.unidadeConteudo ?? 'g');
  const [contentQuantity,setContentQuantity]=useState(selected ? String(selected.conteudoQuantidade) : '');
  const [density,setDensity]=useState(String(selected?.densidadeGramasPorMl ?? 0.92));
  const isVolume=isVolumeContentUnit(contentUnit);
  const equivalentGrams=packageContentToGrams(Number(contentQuantity),contentUnit,isVolume?Number(density):undefined);

  return <form onSubmit={onSubmit} className="space-y-4">
    <Field label="Nome do alimento"><input name="nome" className={input} defaultValue={selected?.nome || ''} placeholder="Ex.: Café, arroz ou óleo" autoFocus/></Field>
    <div className="grid grid-cols-[1fr_96px] gap-3">
      <Field label="Conteúdo por embalagem"><input name="conteudoQuantidade" type="number" min="0.001" step="0.001" className={input} value={contentQuantity} onChange={event=>setContentQuantity(event.target.value)} placeholder="Ex.: 900"/></Field>
      <Field label="Unidade"><select name="unidadeConteudo" className={input} value={contentUnit} onChange={event=>setContentUnit(event.target.value as PackageContentUnit)}>{contentUnits.map(unit=><option key={unit} value={unit}>{unit}</option>)}</select></Field>
    </div>
    {isVolume&&<Field label="Densidade (g/ml)"><input name="densidade" type="number" min="0.001" step="0.001" className={input} value={density} onChange={event=>setDensity(event.target.value)} placeholder="0,92"/><span className="mt-1 block text-xs font-normal text-slate-500">Para óleo, 0,92 g/ml é uma estimativa. Use o peso indicado na embalagem, se houver.</span></Field>}
    {Number(contentQuantity)>0&&Number.isFinite(equivalentGrams)&&equivalentGrams>0&&<p className="rounded-xl bg-slate-50 p-3 text-sm text-slate-600">Equivale a <strong>{fmt(equivalentGrams>=1000?equivalentGrams/1000:equivalentGrams,3)} {equivalentGrams>=1000?'kg':'g'}</strong> por embalagem.</p>}
    <div className="grid grid-cols-2 gap-3"><Field label="Unidade de doação"><select name="unidade" className={input} defaultValue={selected?.unidade || 'pacotes'}>{units.map(unit=><option key={unit}>{unit}</option>)}</select></Field><Field label="Meta em unidades"><input name="meta" type="number" min="1" step="1" className={input} defaultValue={selected?.metaQuantidade || ''} placeholder="300"/></Field></div>
    <button className="w-full rounded-xl bg-slate-900 py-3 font-semibold text-white">{selected?'Salvar alterações':'Adicionar alimento'}</button>
  </form>;
}

function foodContentFromForm(formData:FormData) {
  const conteudoQuantidade=Number(formData.get('conteudoQuantidade'));
  const unidadeConteudo=String(formData.get('unidadeConteudo')) as PackageContentUnit;
  const densidadeGramasPorMl=isVolumeContentUnit(unidadeConteudo)?Number(formData.get('densidade')):null;
  return {
    conteudoQuantidade,
    unidadeConteudo,
    densidadeGramasPorMl,
    pesoPorUnidadeGramas:packageContentToGrams(conteudoQuantidade,unidadeConteudo,densidadeGramasPorMl??undefined),
  };
}
...
export default function App() {
  ...
  function addFood(e: React.FormEvent<HTMLFormElement>) {
    ...
    const nome = String(fd.get('nome')).trim(), conteudo = foodContentFromForm(fd), meta = Number(fd.get('meta'));
    if (!nome || !Number.isFinite(conteudo.pesoPorUnidadeGramas) || conteudo.pesoPorUnidadeGramas <= 0 || meta <= 0) return notify('Preencha os campos com valores válidos.');
    const food: Food = { id: crypto.randomUUID(), campanhaId: data.campaign.id, nome, unidade: String(fd.get('unidade')), ...conteudo, metaQuantidade: meta };
  ...
  function editFood(e: React.FormEvent<HTMLFormElement>) {
    ...
    const nome = String(fd.get('nome')).trim(), conteudo = foodContentFromForm(fd), meta = Number(fd.get('meta'));
    if (!nome || !Number.isFinite(conteudo.pesoPorUnidadeGramas) || conteudo.pesoPorUnidadeGramas <= 0 || meta <= 0) return notify('Preencha os campos com valores válidos.');
    const food = { ...selected, nome, unidade: String(fd.get('unidade')), ...conteudo, metaQuantidade: meta };
  ...
  return <div className="min-h-screen bg-slate-50 text-slate-900">
    <div className="flex min-h-screen">
      ...
      <main className="min-w-0 flex-1">
        <header className="sticky top-0 z-30 border-b border-slate-200 bg-white/95 backdrop-blur"><div className="mx-auto flex max-w-[1500px] items-center justify-between px-4 py-3 sm:px-6 sm:py-4"><div className="flex min-w-0 items-center gap-3"><div className="grid h-10 w-10 shrink-0 place-items-center rounded-xl bg-slate-900 text-white lg:hidden"><Users size={20}/></div><div className="min-w-0"><p className="text-xs font-semibold uppercase tracking-wider text-slate-400">Timbó em Missão</p><h1 className="line-clamp-2 break-words text-base font-bold sm:text-xl">{data.campaign.name}</h1></div></div><div className="ml-2 flex shrink-0 items-center gap-2"><a href={publicUrl} target="_blank" rel="noreferrer" className="hidden items-center gap-2 rounded-xl border border-slate-200 px-3 py-2 text-sm font-semibold hover:bg-slate-50 sm:flex"><ExternalLink size={16}/> Página pública</a><button aria-label="Configurações" onClick={() => setModal('settings')} className="rounded-xl border border-slate-200 p-2.5 hover:bg-slate-50"><Settings size={17}/></button></div></div></header>

        <div className="mx-auto max-w-[1500px] px-4 py-5 pb-28 sm:px-6 sm:py-8 lg:pb-8">
    ...
    <nav aria-label="Navegação principal" className="fixed inset-x-0 bottom-0 z-40 grid grid-cols-5 border-t border-slate-200 bg-white/95 pb-[env(safe-area-inset-bottom)] shadow-[0_-4px_16px_rgba(15,23,42,0.08)] backdrop-blur lg:hidden">
      <MobileNavItem active={view === 'dashboard'} icon={<Home size={20}/>} label="Início" onClick={() => setView('dashboard')}/>
      <MobileNavItem active={view === 'foods'} icon={<Package size={20}/>} label="Alimentos" onClick={() => setView('foods')}/>
      <MobileNavItem active={false} icon={<Plus size={22}/>} label="Registrar" onClick={() => setModal('donation')} disabled={!data.foods.length}/>
      <MobileNavItem active={view === 'history'} icon={<History size={20}/>} label="Histórico" onClick={() => setView('history')}/>
      <MobileNavItem active={view === 'publish'} icon={<Share2 size={20}/>} label="Publicar" onClick={() => setView('publish')}/>
    </nav>
    ...
    {modal === 'food' && <Modal title={selected ? 'Editar alimento' : 'Adicionar alimento'} onClose={() => {setModal(null);setSelected(null)}}><FoodForm key={selected?.id ?? 'new'} selected={selected} onSubmit={selected ? editFood : addFood}/></Modal>}
...
function MobileNavItem({active,icon,label,onClick,disabled=false}:{active:boolean;icon:React.ReactNode;label:string;onClick:()=>void;disabled?:boolean}) { return <button aria-current={active ? 'page' : undefined} disabled={disabled} onClick={onClick} className={`flex min-w-0 min-h-16 flex-col items-center justify-center gap-1 px-1 py-2 text-[11px] font-semibold leading-none transition ${active ? 'text-blue-700' : 'text-slate-500'} ${disabled ? 'cursor-not-allowed opacity-40' : ''}`}><span className="grid h-6 place-items-center">{icon}</span><span className="max-w-full truncate">{label}</span></button>; }
...
function Dashboard({data,stats,chart,recent,onAddFood,onDonation,onFoods,onHistory,onDeleteDonation}:{data:AppData;stats:{total:number;remaining:number;pct:number};chart:any[];recent:Donation[];onAddFood:()=>void;onDonation:()=>void;onFoods:()=>void;onHistory:()=>void;onDeleteDonation:(id:string)=>void}) {
  return <>
    ...
    <section className="mt-8 grid gap-6 lg:grid-cols-5"><div className="min-w-0 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm sm:p-5 lg:col-span-3"><div className="mb-4 flex items-center justify-between gap-3"><div><h3 className="font-bold">Arrecadação por alimento</h3><p className="text-sm text-slate-500">Compare o arrecadado com a meta, em kg.</p></div><BarChart3 size={20} className="shrink-0 text-slate-400"/></div>{chart.length ? <div className="max-h-[480px] overflow-y-auto"><div style={{height: Math.max(260, chart.length * 56)}}><ResponsiveContainer width="100%" height="100%"><BarChart data={chart} layout="vertical" margin={{top: 4, right: 18, left: 8, bottom: 4}} barGap={4}><CartesianGrid strokeDasharray="3 3" horizontal={false}/><XAxis type="number" tickFormatter={(value) => fmt(Number(value))} tickLine={false} axisLine={false}/><YAxis dataKey="name" type="category" width={108} tickLine={false} axisLine={false}/><Tooltip formatter={(value) => `${fmt(Number(value))} kg`}/><Legend/><Bar dataKey="meta" name="Meta" fill="#cbd5e1" radius={[0, 5, 5, 0]} maxBarSize={18}/><Bar dataKey="arrecadado" name="Arrecadado" fill="#059669" radius={[0, 5, 5, 0]} maxBarSize={18}/></BarChart></ResponsiveContainer></div></div> : <Empty text="Você ainda não cadastrou nenhum alimento."/>}</div><div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm lg:col-span-2"><h3 className="font-bold">Metas dos alimentos</h3><p className="mb-5 text-sm text-slate-500">Veja quem está mais perto da meta.</p><div className="space-y-5">{data.foods.map(food => <FoodProgress key={food.id} food={food} donations={data.donations}/>)}{!data.foods.length && <Empty text="Nenhum alimento cadastrado."/>}</div></div></section>
...
function PublicView({data}:{data:AppData}) { const total=data.foods.reduce((s,f)=>s+foodKg(f,data.donations),0), pct=total/data.campaign.metaKg*100; return <div className="min-h-screen bg-slate-50 p-4 sm:p-8"><div className="mx-auto max-w-6xl"><header className="mb-6 flex flex-col items-start gap-3 sm:flex-row sm:items-center sm:justify-between"><div className="min-w-0"><p className="text-xs font-bold uppercase tracking-wider text-slate-500">Timbó em Missão · Ação Social</p><h1 className="break-words text-2xl font-black sm:text-4xl">{data.campaign.name}</h1></div><div className="rounded-full bg-white px-4 py-2 text-sm font-bold shadow-sm">Acompanhamento público</div></header><div className="grid grid-cols-2 gap-3 sm:grid-cols-3 sm:gap-4"><Stat icon={<Target/>} label="Meta" value={`${fmt(data.campaign.metaKg)} kg`}/><Stat icon={<PackagePlus/>} label="Arrecadado" value={`${fmt(total)} kg`}/><div className="col-span-2 sm:col-span-1"><Stat icon={<TrendingUp/>} label="Progresso" value={`${fmt(pct)}%`} positive={pct>=100}/></div></div><div className="mt-5 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm sm:p-5"><div className="flex justify-between gap-3 font-bold"><span>{fmt(total)} / {fmt(data.campaign.metaKg)} kg</span><span>{fmt(pct)}%</span></div><div className="mt-3"><Progress value={pct}/></div></div><div className="mt-6 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm sm:p-5"><h2 className="text-xl font-black">Alimentos</h2><div className="mt-4 divide-y">{data.foods.map(f=><FoodRow key={f.id} food={f} donations={data.donations}/>)}</div>{!data.foods.length&&<Empty text="A arrecadação ainda não começou."/>}</div></div></div>; }
...
function FoodRow({food,donations}:{food:Food;donations:Donation[]}) { const q=donatedQty(food,donations), kg=foodKg(food,donations), p=foodPercent(food,donations); return <div className="flex flex-col gap-3 p-4 sm:flex-row sm:items-center sm:justify-between"><div><p className="font-bold">{food.nome}</p><p className="text-xs text-slate-500">{foodPackagingLabel(food)}</p></div><div className="w-full max-w-xl"><div className="mb-1 flex justify-between text-sm"><span>{fmt(q,0)} / {fmt(food.metaQuantidade,0)} {food.unidade}</span><strong>{fmt(p)}%</strong></div><Progress value={p}/><p className="mt-1 text-xs text-slate-500">{fmt(kg)} kg arrecadados</p></div></div>; }
